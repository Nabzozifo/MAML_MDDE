...
def evaluate_sequential(args, runner, env):

    for _ in range(args.test_nepisode):
        runner.run(test_mode=True)

    if args.save_replay:
        runner.save_replay(env)

    runner.close_env(env)
...
def run_sequential(args, logger):
    ...
    # start training
    tasks = Sampler(args.num_tasks).sample_tasks()
    episode = 0
    ...
    while runner.t_env <= args.t_max:
        ...
        for task in tasks:
            # sample new env
            env.set_task(task)
            episode_batch = runner.run(env, test_mode=False)
            ...
            if buffer.can_sample(args.batch_size):
                new_episode_sample = buffer.sample(args.batch_size)

                # Truncate batch to only filled timesteps
                max_ep_t = new_episode_sample.max_t_filled()
                new_episode_sample = new_episode_sample[:, :max_ep_t]
