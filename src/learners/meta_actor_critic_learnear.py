...
class MetaActorCriticLearner:
    ...
    def train_meta(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        pi = mac_out
        advantages, critic_train_stats, loss_meta = self.train_critic_sequential(self.critic, self.target_critic, batch, rewards,
                                                                                 critic_mask)
        actions = actions[:, :-1]
        ...
        pg_loss_meta = -(((advantages + rewards) * log_pi_taken)
                         * mask).sum() / mask.sum()

        # Optimise agents
        self.agent_optimiser.zero_grad()
        pg_loss_meta.backward()
    ...
    def train_critic_sequential_meta(self, critic, target_critic, batch, rewards, mask):
        ...
        loss_meta = ((masked_td_error + masked_rewards)
                     ** 2).sum() / mask.sum()
        ...
        return masked_td_error, running_log, loss_meta
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        entropy = -th.sum(pi * th.log(pi + 1e-10), dim=-1)
        pg_loss = -((advantages * log_pi_taken +
                    self.args.entropy_coef * entropy) * mask).sum() / mask.sum()
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "meta_advantage_mean", (advantages * mask).sum().item() / mask.sum().item(), t_env)
            self.logger.log_stat("meta_pg_loss", pg_loss.item(), t_env)
            self.logger.log_stat("meta_agent_grad_norm",
                                 grad_norm.item(), t_env)
            self.logger.log_stat("meta_pi_max", (pi.max(
                dim=-1)[0] * mask).sum().item() / mask.sum().item(), t_env)
    ...
    def load_models(self, path):
        ...
        self.target_critic.load_state_dict(self.critic.state_dict())
        self.agent_optimiser.load_state_dict(th.load(
            "{}/agent_opt.th".format(path), map_location=lambda storage, loc: storage))
        self.critic_optimiser.load_state_dict(th.load(
