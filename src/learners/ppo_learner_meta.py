...
class PPOMetaLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.standardise_returns:
            self.ret_ms = RunningMeanStd(
                shape=(self.n_agents, ), device=device)
        if self.args.standardise_rewards:
            self.rew_ms = RunningMeanStd(shape=(1,), device=device)

    def train2(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        old_log_pi_taken = th.log(old_pi_taken)
        ...
        for k in range(self.args.epochs):
            ...
            loss1 = ((rewards * mask + masked_td_error)
                     ** 2).sum() / mask.sum()
            loss2 = ((rewards * mask + masked_td_error)
                     * log_pi_taken).sum() / mask.sum()
    ...
    def update_parameters(self, all_actor_param, all_critic_param, learning_rate):
        ...
        with th.no_grad():
            for param1, param2 in zip(self.agent_params, self.critic_params):
                self.update_function(param1, all_actor_param, learning_rate)
                self.update_function(param2, all_critic_param, learning_rate)
    ...
    def update_function(self, param, all_param, learning_rate):
        ...
        return param - learning_rate * all_param

    def train_meta(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        # Get the relevant quantities
        ...
        for k in range(self.args.epochs):
            ...
            pi = mac_out
            advantages, _, loss_meta = self.train_critic_sequential(self.critic, self.target_critic, batch, rewards,
                                                                    critic_mask)
            advantages = advantages.detach()
            # Calculate policy grad with mask
        ...
        # for item1, item2 in zip(self.agent_params, self.critic_params):
        #     sum1 = sum1+item1.grad
        #     sum2 = sum2+item2.grad
        ...
        #     self.logger.log_stat(
        #         "advantage_mean", (advantages * mask).sum().item() / mask.sum().item(), t_env)
    ...
    def train_critic_sequential_meta(self, critic, target_critic, batch, rewards, mask):
        ...
        if self.args.standardise_returns:
            ...
            target_returns = (target_returns - self.ret_ms.mean) / \
                th.sqrt(self.ret_ms.var)
        ...
        return masked_td_error, target_returns, loss_meta
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for k in range(self.args.epochs):
            ...
            pi = mac_out
            advantages, critic_train_stats, _ = self.train_critic_sequential(self.critic, self.target_critic, batch, rewards,
                                                                             critic_mask)
            advantages = advantages.detach()
            # Calculate policy grad with mask
            ...
            entropy = -th.sum(pi * th.log(pi + 1e-10), dim=-1)
            pg_loss = -((th.min(surr1, surr2) + self.args.entropy_coef *
                        entropy) * mask).sum() / mask.sum()
    ...
    def train_critic_sequential(self, critic, target_critic, batch, rewards, mask):
        ...
        return masked_td_error, running_log, target_returns
